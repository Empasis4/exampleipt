...
export default function Faculty(){
  ...
  const blank={name:'',email:'',department_id:'',position:'',experience:'',status:'Active'};
  ...
  const remove=async(f)=>{ if(confirm('Delete faculty?')){ await axios.delete(`/faculties/${f.id}`); await fetchAll(); } };
                    ...
                    <tr key={f.id}>
                      ...
                      <td style={{padding:8,border:'1px solid #e5e7eb'}}>
                        <span style={{display:'inline-flex',gap:6}}>
                          ...
                          <IconButton title="Delete" onClick={()=>remove(f)}><TrashIcon/></IconButton>
                        </span>
                      </td>
                    </tr>
                  );
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.2)',display:'flex',alignItems:'center',justifyContent:'center'}}>
          <div style={{background:'#fff',padding:16,borderRadius:8,width:520}}>
            ...
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:12}}>
              ...
              <input type="number" min="0" step="1" placeholder="Experience (years)" value={form.experience} onChange={e=>setForm({...form,experience:e.target.value})}/>
